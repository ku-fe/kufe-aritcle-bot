...
export async function handleArticleModalSubmit(
  ...
  try {
      ...
      interaction.message?.components
        ...
        .filter((component) => {
          if (component instanceof ButtonComponent) {
            return (
              component.customId?.startsWith(TAG_BUTTON_PREFIX) &&
    ...
    const categories = selectedButtons.map((button) => {
      ...
      return ARTICLE_CATEGORIES[tag.key as keyof typeof ARTICLE_CATEGORIES]
        .value;
    });
    ...
    if (existingArticle) {
      const categoryLabels = existingArticle.categories
        .map(
          (cat) =>
            ARTICLE_CATEGORIES[
              cat.toUpperCase() as keyof typeof ARTICLE_CATEGORIES
            ].label,
        )
        .join(', ');
